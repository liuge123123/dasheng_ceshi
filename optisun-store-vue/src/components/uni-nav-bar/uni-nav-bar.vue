				...
				<view @tap="onClickLeft" class="uni-navbar__header-btns uni-navbar__header-btns-left"
					...
					<slot name="left">
						<view class="uni-navbar__content_view">
							<uni-icons :color="themeColor" :type="leftIcon" size="20" v-if="backShow && leftIcon.length > 0" @click="goBack" />
							<uni-icons :color="themeColor" type="home" size="20" v-if="showHomeIcon" @click="goHome" />
						</view>
						<view :class="{ 'uni-navbar-btn-icon-left': !leftIcon.length > 0 }" class="uni-navbar-btn-text"
							v-if="backShow && leftText.length">
					...
					<slot>
						<view class="uni-navbar__header-container-inner">
							<text class="uni-nav-bar-text uni-ellipsis-1"
								:style="{color: themeColor }">{{ $t(showTitle) }}</text>
						</view>
					</slot>
				</view>
	...
	export default {
		...
		computed: {
			...
			showTitle(){
				...
				if(this.$Route.style.navigationBarTitleText){
					return this.$Route.style.navigationBarTitleText.replace(/%/g, '')
				}
				return ''
			},
			showHomeIcon(){
