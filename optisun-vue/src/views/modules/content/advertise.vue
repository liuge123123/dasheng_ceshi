  ...
  <div class="mod-config">
    <el-card v-if="!addOrUpdateVisible">
      ...
      <el-form :inline="true" size="small">
        <el-form-item>
          <el-button icon="el-icon-plus" v-if="isAuth('content:advertise:save')" type="primary"
                     @click="addOrUpdateHandle()">新增
          </el-button>
          <el-button icon="el-icon-delete" v-if="isAuth('content:advertise:delete')" type="danger"
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="操作">
          <template slot-scope="scope">
            <el-button icon="el-icon-edit" v-if="isAuth('content:advertise:update')" type="text" size="small"
                       @click="addOrUpdateHandle(scope.row.id)">修改
            </el-button>
            <el-button icon="el-icon-delete" v-if="isAuth('content:advertise:delete')" type="text" size="small"
...
export default {
  data() {
    return {
      ...
      searchFormOptions: {
        filedList: [
          {filedCode: 'key', filedName: '关键字', filedType: '1',placeholder:'图片名称'}
        ]
      }
    }
  },
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl('/content/advertise/list'),
        method: 'get',
    ...
    deleteHandle(id) {
      ...
      this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
        ...
        this.$http({
          url: this.$http.adornUrl('/content/advertise/delete'),
          method: 'post',
