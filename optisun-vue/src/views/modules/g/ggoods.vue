  ...
  <div class="mod-config">
    <el-card v-if="!addOrUpdateVisible">
      ...
      <el-form :inline="true" size="small">
        <el-form-item>
          <el-button icon="el-icon-plus" v-if="isAuth('g:ggoods:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
          <el-button icon="el-icon-delete" v-if="isAuth('g:ggoods:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
        </el-form-item>
      </el-form>
      <el-table
        :data="dataList"
...
<script>
  import AddOrUpdate from './ggoods-add-or-update'
  ...
  export default {
      ...
      return {
        ...
        searchFormOptions: {
          filedList: [
            ...
            { filedCode: 'key', filedName: '关键字', filedType: '1',placeholder:'商品名'}
          ]
        }
      }
    },
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/g/ggoods/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/g/ggoods/delete'),
            method: 'post',
