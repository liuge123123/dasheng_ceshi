<template>
  <div class="mytransfer-container">
    <div class="box">
      <div class="title">
        <span :class="{active: leftSelect === data1.length}" @click="selectAll(data1)">待选字段</span>
      ...
      <div @drop="dropPub($event, 2)" @dragover.prevent class="left">
        <div
          ...
          :key="index">{{ item.id }}-{{ item.value }}<label v-if="item.sysField == 1" style="color: red; height:30px; line-height: 30px">*</label>
        </div>
      </div>
    </div>

    <div class="middle">
      <div v-on:click="moveItem(data2, data1, 1)" :class="{active: rightSelect > 0}"><i class="el-icon-arrow-left"></i>
      </div>
      <div v-on:click="moveItem(data1, data2, 2)" :class="{active: leftSelect > 0}"><i class="el-icon-arrow-right"></i>
      </div>
    </div>

    <div class="box">
      <div class="title">
        <span :class="{active: rightSelect === data2.length}" @click="selectAll(data2)">已选字段</span>
...
export default {
  ...
  methods: {
    ...
    drop(event, handle, target, type) {
      ...
      this.resort(event, target, type, flag)
    },
    resort(event, target, type, isOther) {
      ...
      if (type == 1) {
        ...
        if (isOther) {
          this.$emit('dropLeftToRight', tempId)
        }
      }
      if (type == 2) {
        let tempId = event.dataTransfer.getData('id')
        ...
        this.$emit('input', emptyArray)
        if (isOther) {
          this.$emit('dropRightToLeft', tempId)
        }
      }
      // 两个框拖拽数据
...
.mytransfer-container {
  ...
  .box .title span:first-child {
    ...
    background: #f5f7fa;
    background-image: url(../../../src/assets/img/check-box-outline-blank.png);
  ...
  .box .title span:first-child.active {
    background-image: url(../../../src/assets/img/check_box.png);
  }
